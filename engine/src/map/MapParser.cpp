...
)
{
	...
	try 
	{
		...
		if (p.extension() != ".json" && p.extension() != ".tmj")
			throw std::runtime_error("ERROR MapParser::start -> Bad extension for " + path);
		if (!std::filesystem::exists(p))
			throw std::runtime_error("ERROR MapParser::start -> File not found : " + path);
		std::ifstream file(path);
		if (!file)
			throw std::runtime_error("ERROR MapParser::start -> failed opening : " + path);
		nlohmann::json	data;
		file >> data;
