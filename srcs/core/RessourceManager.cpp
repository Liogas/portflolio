...
TTF_Font	*RessourceManager::getFont(const std::string &name, int size)
{
...
std::string	RessourceManager::getFontsPath()
{
	static std::string path;
	...
	if (path.empty())
	{
		...
		while (!std::filesystem::exists(exePath / "assets"))
		{
			...
			if (exePath == exePath.root_path())
				throw std::runtime_error(
					"ERROR RessourceManager::getFontsPath : assets folder not found"
				);
		}
